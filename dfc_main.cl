...
__kernel void Traversej(
    ...
    int st = 0;
    for (; st <= Zinv_block_size - step; st += step) {
        if (*stop_flag != -1) {
            return;
        }

        int j = st + gid + 1 + cur_j;

        uint k[8];

        uint t2jx[8];
        convert_l5_to_i8(t2[j * 2].n, t2jx);
        uint t2jy[8];
        convert_l5_to_i8(t2[j * 2 + 1].n, t2jy);
        uint inv[8];
        convert_l5_to_i8(ZinvTree[j - cur_j - 1].n, inv);
    ...
    if (st != Zinv_block_size) {
        if (gid < Zinv_block_size - st) {
            if (*stop_flag != -1) {
                return;
            }
